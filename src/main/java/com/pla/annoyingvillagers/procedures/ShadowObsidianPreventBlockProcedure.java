    ...
    public static void execute(LevelAccessor levelaccessor, final double d0, final double d1, final double d2) {
        ...
        if (levelaccessor instanceof Level) {
            ...
            if (!level.isClientSide()) {
                level.playSound((Player)null, new BlockPos(d0, d1, d2), (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("minecraft", "block.deepslate.place")), SoundSource.BLOCKS, 1.0F, (float)Mth.nextDouble(AnnoyingVillagers.randomSource, 0.95D, 1.0D));
            } else {
        ...
        if (levelaccessor instanceof Level) {
            level = (Level)levelaccessor;
            if (!level.isClientSide()) {
                level.playSound((Player)null, new BlockPos(d0, d1, d2), (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("annoyingvillagers", "ob_place")), SoundSource.BLOCKS, (float)Mth.nextDouble(AnnoyingVillagers.randomSource, 0.2D, 1.0D), 1.0F);
            } else {
            ...
            public void run() {
                ...
                if (levelaccessor1 instanceof Level) {
                    ...
                    if (!level1.isClientSide()) {
                        level1.playSound((Player)null, new BlockPos(d0, d1, d2), (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("minecraft", "block.stone.break")), SoundSource.BLOCKS, 1.0F, (float)Mth.nextDouble(AnnoyingVillagers.randomSource, 1.0D, 1.1D));
                    } else {
