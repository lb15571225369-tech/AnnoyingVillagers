...
public final class AnnoyingVillagersSpawnConfig {
    ...
    public static java.util.List<Entry> getEntries() { return ENTRIES; }

    public static int getRawWeight(String entityId) {
        Entry entry = entryByEntityId.get(entityId);
        if (entry == null) return 0;
        if (!entry.groupSizeConfigurable()) {
            ForgeConfigSpec.IntValue v = weightValueByEntityId.get(entityId);
            return v != null ? v.get() : entry.defaultConfig().weight();
        }
        return getSpawnConfig(entityId).weight();
    }

    public static int getRawMinCount(String entityId) {
        Entry entry = entryByEntityId.get(entityId);
        if (entry == null) return 1;
        return getSpawnConfig(entityId).minCount();
    }

    public static int getRawMaxCount(String entityId) {
        Entry entry = entryByEntityId.get(entityId);
        if (entry == null) return 1;
        return getSpawnConfig(entityId).maxCount();
    }

    public static void setSpawn(String entityId, int weight, int minCount, int maxCount) {
        Entry entry = entryByEntityId.get(entityId);
        if (entry == null) return;
        int w = Math.max(0, Math.min(1000, weight));
        if (!entry.groupSizeConfigurable()) {
            ForgeConfigSpec.IntValue v = weightValueByEntityId.get(entityId);
            if (v != null) v.set(w);
            return;
        }
        int mn = Math.max(1, Math.min(64, minCount));
        int mx = Math.max(1, Math.min(64, maxCount));
        if (mx < mn) mx = mn;
        ForgeConfigSpec.ConfigValue<List<? extends Number>> tv = tripleValueByEntityId.get(entityId);
        if (tv != null) tv.set(List.of(w, mn, mx));
    }

    public static boolean isGroupSizeConfigurable(String entityId) {
        Entry e = entryByEntityId.get(entityId);
        return e != null && e.groupSizeConfigurable();
    }
}