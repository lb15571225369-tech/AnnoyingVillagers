    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand interactionHand) {
        if (player.getItemBySlot(EquipmentSlot.HEAD).getItem() == Blocks.AIR.asItem()) {
            ...
            if (!player.level().isClientSide()) {
                player.displayClientMessage(Component.translatable("message.annoyingvillagers.villager_head_equipped"), false);
            }
        }
            ...
            if (TeamUtil.isInTeam(entity, "villagers")) {
                ...
                if (entity instanceof Player player && !player.level().isClientSide()) {
                    player.displayClientMessage(
                            Component.translatable("message.annoyingvillagers.villager_head_removed"),
                            false
                    );
                }
            }
